...
using Hayalpc.Dcb.Panel.Internal.Helpers.Interfaces;
...
namespace Hayalpc.Dcb.Panel.Internal.Services
{
    public class UserService : BaseService<User>, IUserService
    {
        ...
        public IDataResult<SessionModel> Login(LoginRequest request)
        {
            ...
            if (user != null)
            {
                if (user.WrongLoginTryCount > Library.Common.Helpers.WebSiteDefinitionHelper.WrongPasswordCount)
                {
                    return new ErrorDataResult<SessionModel>("AccountBlockedMustChangePassword");
                }

                if (DateTime.Now.AddDays(-Library.Common.Helpers.WebSiteDefinitionHelper.PasswordExpireDay) >= user.LastPasswordChangeDate)
                {
                ...
                userDto.UserRoles = userRole.GetByUserId(user.Id).Convert<List<Library.Common.Dtos.UserRoleDto>>();
                ...
                session.Permissions = userPermissions.Convert<List<Library.Common.Dtos.RolePermissionDto>>();
            ...
            else
            {
                ...
                if (user != null)
                {
                    try
                    {
                        if (Library.Common.Helpers.WebSiteDefinitionHelper.WrongPasswordCount >= user.WrongLoginTryCount)
                        {
            ...
            try
            {
                if (!repository.GetQuery(u => u.Email == user.Email).Any())
                {
                    ...
                    if (resRes.IsSuccess)
                    {
                        ...
                        var userRoleId = repository.GetQuery<Role>(x => x.Type == Dcb.Common.Enums.UserType.User).Select(x => x.Id).FirstOrDefault();
                        ...
                        mailHelper.SendResetPassword(user.Convert<UserDto>(), resRes.Data.Convert<Library.Common.Dtos.ResetPasswordDto>());
        ...
        public Result AccountChangePassword(ChangePassword changePassword)
        {
            if (changePassword.NewPassword == changePassword.NewPasswordConfirm)
            {
                ...
                {
                    var user = userRes.Data;
                    user.Password = Library.Common.Helpers.Md5Helper.Get(user.Email + ":" + changePassword.NewPassword);

                    var oldPasswords = repository.GetQuery<UserLog>(x => x.ActionType == "passwordChange")
                        .OrderByDescending(x => x.Id)
                        .Take(Library.Common.Helpers.WebSiteDefinitionHelper.WrongPasswordCount)
        ...
        public Result UpdatePassword(PasswordRequest passwordRequest)
        {
            if (passwordRequest.Password == passwordRequest.PasswordConfirm)
            {
                    ...
                    && resetPasswordRes.Data.Status == Status.New)
                {
                    ...
                    {
                        var user = userRes.Data;
                        user.Password = Library.Common.Helpers.Md5Helper.Get(user.Email + ":" + passwordRequest.Password);
