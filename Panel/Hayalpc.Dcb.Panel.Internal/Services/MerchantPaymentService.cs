...
namespace Hayalpc.Dcb.Panel.Internal.Services
{
    public class MerchantPaymentService : BaseService<MerchantPayment>, IMerchantPaymentService
    {
        ...
        public IResult Calculate(long[] merchantIds)
        {
            ...
            if (merchantIds != null)
            {
                items = repository.GetQuery<CarrierCollectionItem>(x => x.Status == CarrierCollectionStatus.Active && merchantIds.Contains(x.MerchantId)).ToList();
            }
            else
            {
                items = repository.GetQuery<CarrierCollectionItem>(x => x.Status == CarrierCollectionStatus.Active).ToList();
            }
            ...
            if (items.Count > 0)
            {
                try
                {
                    ...
                    foreach (var merchant in merchants)
                    {
                        ...
                        {
                            MerchantId = merchant,
                            Status = MerchantPaymentStatus.Processing,
                    ...
                    foreach (var item in items)
                    {
                        ...
                        item.Status = CarrierCollectionStatus.PaymentPending;
                    ...
                    {
                        merchantPayment.Status = MerchantPaymentStatus.ApprovePending;
