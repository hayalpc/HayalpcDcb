...
namespace Hayalpc.Dcb.Panel.Internal.Services
{
    public abstract class BaseService<TEntity> : IBaseService<TEntity> where TEntity : class, IHpModel
    {
        ...
        private TableDefinition GetTableDefinitionFromCache(string modelName, Library.Common.Enums.ActionType actionType)
        {
            ...
            return memoryCache.GetOrCreate(cacheKey, (entry) =>
                {
                    ...
                    x.Status == Library.Common.Enums.Status.Active &&
                    x.ActionType == actionType).FirstOrDefault();
                });
        }

        private bool HasTableHistory(string modelName, Library.Common.Enums.ActionType actionType, long dataId = 0)
        {
        ...
        public virtual IDataResult<TEntity> Add(TEntity model)
        {
            try
            {
                ...
                var tableDefinition = GetTableDefinitionFromCache(modelName, Library.Common.Enums.ActionType.Insert);
                if (tableDefinition != null)
                {
                    if (!HasTableHistory(modelName, Library.Common.Enums.ActionType.Insert))
                    {
                        var tableHistory = new TableHistory
                        {
                            ...
                            ActionType = Library.Common.Enums.ActionType.Insert,
                            ActionDetail = "Insert",
                        ...
                        if (tableDefinition.RoleId2 > 0)
                        {
                            try
                            {
                                ...
                                var userBulletin = GetByRoleId(tableDefinition.RoleId2 ?? 0).Select(x => new UserBulletin
                                {
                                    ...
                                    Title = "Talep Onay",
                                    Message = str,
                                    Type = Library.Common.Enums.UserBulletinType.Warning,
                                    Status = Library.Common.Enums.UserBulletinStatus.Unread,
        ...
        public virtual IDataResult<TEntity> Update(TEntity model)
        {
            try
            {
                ...
                var tableDefinition = GetTableDefinitionFromCache(modelName, Library.Common.Enums.ActionType.Update);
                if (tableDefinition != null)
                {
                    if (!HasTableHistory(modelName, Library.Common.Enums.ActionType.Update, model.Id))
                    {
                        var tableHistory = new TableHistory
                        {
                            MerchantId = RequestHelper.MerchantId,
                            TableDefinitionId = tableDefinition.Id,
                            ActionType = Library.Common.Enums.ActionType.Update,
                            Status = tableDefinition.RoleId1 > 0 ? TableHistoryStatus.New : TableHistoryStatus.Log,
                            ActionDetail = "Update",
                            ModelName = tableDefinition.ModelName,
                            DataId = model.Id,
