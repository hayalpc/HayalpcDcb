                        ...
                        if (result.Success)
                        {
                            if (model.Status == TableHistoryStatus.Approved)
                            {
                                try
                                {
                                    ...
                                    if (tableDefinition.ActionType == Library.Common.Enums.ActionType.Insert)
                                    {
                                        var insertData = (IHpModel)JsonConvert.DeserializeObject(model.NewData, modelType);
                                        modelCusRes = base.AddCustom(insertData);
                                    }
                                    else if (tableDefinition.ActionType == Library.Common.Enums.ActionType.Update)
                                    {
                                        var updateData = (IHpModel)JsonConvert.DeserializeObject(model.NewData, modelType);
                                        modelCusRes = base.UpdateCustom(updateData);
                                    }
                                    else if (tableDefinition.ActionType == Library.Common.Enums.ActionType.Delete)
                                    {
                                    }
                                    else if (tableDefinition.ActionType == Library.Common.Enums.ActionType.Approve)
                                    {
                                    }
                                    else
