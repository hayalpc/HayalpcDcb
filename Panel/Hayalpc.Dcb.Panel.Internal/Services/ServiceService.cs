...
namespace Hayalpc.Dcb.Panel.Internal.Services
{
    public class ServiceService : BaseService<Service>, IServiceService
    {
        ...
        {
            var blobFiles = repository.GetQuery<BlobFile>(x => x.DataId == model.Id && x.Type == Dcb.Common.Enums.BlobFileType.Service && x.Status == Library.Common.Enums.Status.Active).ToList();
